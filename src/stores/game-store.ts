...
type GameState = {
  ...
  register: (username: string, password: string, teamName: string, emblemId: string, country: string) => Promise<boolean>;
  login: (username: string, password: string) => Promise<boolean>;
  updateProfile: (values: { playerName: string; teamName: string; emblemId: string; country: string }) => Promise<boolean>;
  logout: () => Promise<void>;
...
export const useGameStore = create<GameState>((set, get) => ({
  ...
  register: async (username, password, teamName, emblemId, country) => {
    try {
      const data = await apiRequest<ApiUserResponse>("/api/auth/register", { method: "POST", body: JSON.stringify({ username, password, teamName, emblemId, country }) });
  ...
  updateProfile: async ({ playerName, teamName, emblemId, country }) => {
    try {
      const data = await apiRequest<ApiUserResponse>("/api/auth/profile", { method: "PATCH", body: JSON.stringify({ playerName, teamName, emblemId, country }) });
      const user = data.user ? normalizeAccount(data.user) : undefined;

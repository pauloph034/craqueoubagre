...
export async function PATCH(request: Request) {
  ...
  const body = (await request.json().catch(() => ({}))) as { playerName?: string; teamName?: string; emblemId?: string };
  ...
  if (playerName.length < 3 || teamName.length < 3 || !isValidEmblemId(emblemId)) {
    return NextResponse.json({ error: "Use nome do jogador, nome do time e um escudo valido." }, { status: 400 });
  }
  ...
  const user = await updateStoredUser(currentUser.username, { playerName, teamName, emblemId });

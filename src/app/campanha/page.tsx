...
type MatchSpeed = "normal" | "rapida" | "ultra";

const matchSpeedDelay: Record<MatchSpeed, number> = {
  normal: 1200,
  rapida: 750,
  ultra: 420
};
...
function BracketPair({ pair, customTeamName, emblemId }: { pair: KnockoutPairView; customTeamName: string; emblemId?: string }) {
  return (
          ...
          <div className="min-w-0">
            ...
            {match.tacticalContext && (
              <p className="mt-2 text-xs text-slate-400">
                Forca {match.tacticalContext.userStrength} x {match.tacticalContext.opponentStrength} · {match.tacticalContext.summary}
              </p>
            )}
          </div>
...
function MatchSpeedControl({ speed, onSpeedChange }: { speed: MatchSpeed; onSpeedChange: (speed: MatchSpeed) => void }) {
  return (
    <div className="flex border border-white/10 bg-night/70 p-1">
      {(["normal", "rapida", "ultra"] as MatchSpeed[]).map((item) => (
        <button
          key={item}
          type="button"
          className={`rounded-sm px-3 py-1 text-[10px] font-black uppercase tracking-[0.14em] transition ${speed === item ? "bg-electric text-night" : "text-slate-300 hover:bg-white/10"}`}
